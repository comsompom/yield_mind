# Run "weave rollup launch" inside WSL (Weave is not available in Windows PowerShell).
...
Write-Host "Starting WSL and running 'weave rollup launch' in: $wslPath" -ForegroundColor Cyan
...
$bashCmd = 'export PATH=$PATH:$HOME/.local/bin:/usr/local/go/bin && cd ''' + $wslPath + ''' && weave rollup launch'
